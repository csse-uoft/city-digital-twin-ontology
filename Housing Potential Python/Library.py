...
synthetic = Graph()
synthetic.bind('tor', TORONTO)
synthetic.bind('hp', HP)
...
for _, row in tpl_df.iterrows():
    branch_uri = TORONTO[f"library_{row['_id']}"]
    g.add((branch_uri, GENPROP.hasIdentification, Literal(row['_id'], datatype=XSD.string)))
    ...
    branch_add_uri = TORONTO[f"addLibrary_{row['_id']}"]
    ...
    site_uri = TORONTO[f"library_site_{row['_id']}"]
    g.add((site_uri, RDF.type, CDT.LibrarySite))
    g.add((site_uri, ORG.siteAddress, branch_uri))
    g.add((site_uri, RDFS.subClassOf, CDT.Site))
    g.add((site_uri, GENPROP.hasName, Literal(row['BranchName'], datatype=XSD.string)))

    g.add((CDT.Site, RDFS.subClassOf, ORG.Site))
    g.add((branch_uri, ORG.hasSite, site_uri))

    floor_uri = TORONTO[f"library_site_{row['_id']}FloorArea"]
    floor_measure_uri = TORONTO[f"library_site_{row['_id']}FloorAreaMeasure"]

    g.add((site_uri, HP.hasFloorArea, floor_uri))
    g.add((floor_uri, RDF.type, HP.FloorArea))
    g.add((floor_uri, I72.hasValue, floor_measure_uri))
    g.add((floor_measure_uri, I72.hasNumericalValue, Literal(int(row['SquareFootage']), datatype=XSD.integer)))
    g.add((floor_measure_uri, I72.hasUnit, square_foot))
    ...
    if row['KidsStop'] == 1:
        kid_uri = TORONTO[f"kids_stop_{row['_id']}"]
        g.add((kid_uri, RDF.type, TORONTO.TorLibraryService))
        g.add((branch_uri, CDT.providesService, kid_uri))
        g.add((kid_uri, HP.providedFromSite, site_uri))
        g.add((kid_uri, GENPROP.hasName, Literal('KidsStop', datatype=XSD.string)))

    if row['TPLNIA'] == 1:
        tplnia_uri = TORONTO[f"tplnia_serv_{row['_id']}"]
        g.add((tplnia_uri, RDF.type, TORONTO.TorLibraryService))
        g.add((branch_uri, CDT.providesService, tplnia_uri))
        g.add((tplnia_uri, HP.providedFromSite, site_uri))
        g.add((tplnia_uri, GENPROP.hasName, Literal('TPLNIA', datatype=XSD.string)))

    if row['LeadingReading'] == 1:
        lr_uri = TORONTO[f"leadingReading_serv_{row['_id']}"]
        g.add((lr_uri, RDF.type, TORONTO.TorLibraryService))
        g.add((branch_uri, CDT.providesService, lr_uri))
        g.add((lr_uri, HP.providedFromSite, site_uri))
        g.add((lr_uri, GENPROP.hasName, Literal('LeadingReading', datatype=XSD.string)))

    if row['CLC'] == 1:
        clc_uri = TORONTO[f"clc_serv_{row['_id']}"]
        g.add((clc_uri, RDF.type, TORONTO.TorLibraryService))
        g.add((branch_uri, CDT.providesService, clc_uri))
        g.add((clc_uri, HP.providedFromSite, site_uri))
        g.add((clc_uri, GENPROP.hasName, Literal('CLC', datatype=XSD.string)))

    if row['DIH'] == 1:
        dih_uri = TORONTO[f"dih_serv_{row['_id']}"]
        g.add((dih_uri, RDF.type, TORONTO.TorLibraryService))
        g.add((branch_uri, CDT.providesService, dih_uri))
        g.add((dih_uri, HP.providedFromSite, site_uri))
        g.add((dih_uri, GENPROP.hasName, Literal('DIH', datatype=XSD.string)))

    if row['TeenCouncil'] == 1:
        tc_uri = TORONTO[f"teenCouncil_serv_{row['_id']}"]
        g.add((tc_uri, RDF.type, TORONTO.TorLibraryService))
        g.add((branch_uri, CDT.providesService, tc_uri))
        g.add((tc_uri, HP.providedFromSite, site_uri))
        g.add((tc_uri, GENPROP.hasName, Literal('TeenCouncil', datatype=XSD.string)))

    if row['YouthHub'] == 1:
        yh_uri = TORONTO[f"youthHub_serv_{row['_id']}"]
        g.add((yh_uri, RDF.type, TORONTO.TorLibraryService))
        g.add((branch_uri, CDT.providesService, yh_uri))
        g.add((yh_uri, HP.providedFromSite, site_uri))
        g.add((yh_uri, GENPROP.hasName, Literal('YouthHub', datatype=XSD.string)))

    if row['AdultLiteracyProgram'] == 1:
        alp_uri = TORONTO[f"adultLiteracy_serv_{row['_id']}"]
        g.add((alp_uri, RDF.type, TORONTO.TorLibraryService))
        g.add((branch_uri, CDT.providesService, alp_uri))
        g.add((alp_uri, HP.providedFromSite, site_uri))
        g.add((alp_uri, GENPROP.hasName, Literal('AdultLiteracyProgram', datatype=XSD.string)))


    service_uri = TORONTO[f"library_service{row['_id']}"]
    service_capacity_uri = TORONTO[f"library_service{row['_id']}Capacity"]
    service_capacity_measure_uri = TORONTO[f"library_service{row['_id']}CapacityMeasure"]
    service_capacity_use_uri = TORONTO[f"library_service{row['_id']}CapacityUse"]
    service_capacity_use_measure_uri = TORONTO[f"library_service{row['_id']}CapacityUseMeasure"]

    avail_capacity_use_uri = TORONTO[f"library_service{row['_id']}AvailCapacity"]
    avail_capacity_use_measure_uri = TORONTO[f"library_service{row['_id']}AvailCapacityMeasure"]


    g.add((branch_uri, CDT.providesService, service_uri))
    g.add((service_uri, RDF.type, TORONTO.TorLibraryService))
    ...
    synthetic.add((service_capacity_measure_uri, I72.hasNumericalValue, Literal(1, datatype=XSD.integer)))
    synthetic.add((service_capacity_measure_uri, I72.hasUnit, square_metre))

    synthetic.add((service_uri, RES.capacityInUse, service_capacity_use_uri))
...
for feature in geojson_data["features"]:
    ...
    if library:
        ...
        if "capacity" in props:
            g.add((site_uri, CDT.maxOccupancy, Literal(int(props['capacity'].split()[0]), datatype=XSD.integer)))
        ...
        if "@id" in props:
            id = props['@id'].split('/')[1]
            branch_uri = TORONTO[f"library_{id}"]
            g.add((branch_uri, GENPROP.hasIdentification, Literal(id, datatype=XSD.string)))
            g.add((branch_uri, RDF.type, CDT.Library))

            site_uri = TORONTO[f"library_Site_{id}"]
            g.add((site_uri, RDF.type, CDT.LibrarySite))
            g.add((site_uri, RDFS.subClassOf, CDT.Site))
            g.add((branch_uri, ORG.hasSite, site_uri))

            branch_add_uri = TORONTO[f"addLibrary_{id}"]
            g.add((branch_add_uri, RDF.type, CONTACT.Address))
            g.add((branch_uri, CONTACT.hasAddress, branch_add_uri))
        ...
        if "addr:housename" in props:
            ...
            g.add((site_uri, GENPROP.hasName, Literal(props['addr:housename'], datatype=XSD.string)))
        ...
        if "name" in props:
            ...
            g.add((site_uri, GENPROP.hasName, Literal(props['name'], datatype=XSD.string)))
...
synthetic_ttl = "synthetic_libraries.ttl"
g.serialize(destination=ttl_output_path, format="turtle")
synthetic.serialize(destination=synthetic_ttl, format="turtle")
