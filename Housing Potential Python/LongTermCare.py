...
for idx, row in df.iterrows():
    ...
    g.add((toronto["seniorcare_service" + objectid], RDF.type, toronto.TorLongTermCareService))
    ...
    g.add((toronto["seniorcare_service" + objectid + "CapacityMeasure"], iso21972.hasNumericalValue, Literal(round(row['BEDS_x']))))
    ...
    g.add((toronto["seniorcare_service_site_location" + objectid], geo.asWKT, Literal(shapely.to_wkt(make_valid(shapely.geometry.shape(row["geometry"])), rounding_precision=-1), datatype=geo.wktLiteral)))
    
    g2.add((toronto["seniorcare_service" + objectid], res.capacityInUse, toronto["seniorcare_service" + objectid + "CapacityUse"]))
    g2.add((toronto["seniorcare_service" + objectid + "CapacityUse"], RDF.type, hp.NumberOfLongTermCareResidents))
    ...
    g2.add((toronto["seniorcare_service" + objectid], res.hasAvailableCapacity, toronto["seniorcare_service" + objectid + "CapacityAvail"]))
    g2.add((toronto["seniorcare_service" + objectid + "CapacityAvail"], RDF.type, hp.NumberOfLongTermCareBedsAvailable))
    g2.add((toronto["seniorcare_service" + objectid + "CapacityAvail"], iso21972.hasValue, toronto["seniorcare_service" + objectid + "CapacityAvailMeasure"]))

    g2.add((toronto["seniorcare_service" + objectid + "CapacityAvailMeasure"], RDF.type, iso21972.Measure))
    g2.add((toronto["seniorcare_service" + objectid + "CapacityAvailMeasure"], iso21972.hasNumericalValue, Literal(round(row['BEDS_x']) - round(row['Fake occupancy']))))
    g2.add((toronto["seniorcare_service" + objectid + "CapacityAvailMeasure"], iso21972.hasUnit, iso21972.population_cardinality_unit))
