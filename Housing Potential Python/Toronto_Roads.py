...
TRANSPORT = Namespace("https://standards.iso.org/iso-iec/5087/-3/ed-1/en/ontology/TransportNetwork#")
...
g.bind("geo", GEO)
g.bind("i50873", TRANSPORT)
g.bind("loc", GEO_LOC)
g.bind("mer", PARTWHOLE)
...
for road_name, group in road_groups:
    ...
    for _, row in group.iterrows():
        ...
        if not (maxy >= toronto_bounds["lat_min"] and miny <= toronto_bounds["lat_max"] and
                maxx >= toronto_bounds["lon_min"] and minx <= toronto_bounds["lon_max"]):
            continue
        ...
        g.add((road_link_uri, TRANSPORT.usedBy, road_user_uri))
        g.add((road_user_uri, TRANSPORT.uses, road_link_uri))
        ...
        if pd.notna(speed_limit):
            speed_uri = ORN[f"speed_{road_id}"]
        ...
        if pd.notna(road_class and speed_limit and num_lanes): # ORN Capacity
            ...
            service_uri = ORN[f"roadLink_{road_id}Service"]
            ...
            synthetic.add((capacity_uri, RDF.type, HP.VehicleThroughputRate))
            synthetic.add((capacity_uri, I72.hasValue, capacity_measure_uri))
            synthetic.add((capacity_measure_uri, I72.hasNumericalValue, Literal(capacity, datatype=XSD.integer)))
            synthetic.add((capacity_measure_uri, I72.hasUnit, HP.vehicles_per_hour))

            synthetic.add((road_link_uri, RES.capacityInUse, in_use_uri))
            synthetic.add((service_uri, RES.capacityInUse, in_use_uri))

            synthetic.add((in_use_uri, RDF.type, HP.VehicleThroughputRate))
            synthetic.add((in_use_uri, I72.hasValue, in_use_measure_uri))
            synthetic.add((in_use_measure_uri, I72.hasNumericalValue, Literal(capacity_use,
                                                                      datatype=XSD.integer)))
            synthetic.add((in_use_measure_uri, I72.hasUnit, HP.vehicles_per_hour))


            synthetic.add((road_link_uri, RES.hasAvailableCapacity, avail_uri))
            synthetic.add((service_uri, RES.hasAvailableCapacity, avail_uri))

            synthetic.add((avail_uri, RDF.type, HP.AvailableVehicleThroughputRate))
            synthetic.add((avail_uri, I72.hasValue, avail_measure_uri))
            synthetic.add((avail_measure_uri, I72.hasNumericalValue, Literal(capacity - capacity_use,
                                                                      datatype=XSD.integer)))
            synthetic.add((avail_measure_uri, I72.hasUnit, HP.vehicles_per_hour))
        ...
        if pd.notna(structure_type):
            road_seg_uri = ORN[f"road_seg_{road_id}"]
...
synthetic_file = 'road_synthetic.ttl'
g.serialize(destination=output_file, format="turtle")
synthetic.serialize(destination=synthetic_file, format="turtle")
