...
- A group of RoadLinks sharing the same official street name, route number, route name, or generated unnamed-highway key → transnet:Road
...
The ORN data is grouped using a generated `road_group_key` to identify the collection of RoadLinks that form a single Road. Named roads are grouped by `FULL_STREET_NAME`. However, the updated mapping also keeps unnamed highway-like road elements that would otherwise be skipped because their official street name is missing. This is done because some highways, ramps, or route-numbered road elements may not have `FULL_STREET_NAME` values even though they are still meaningful transportation network entities.

The grouping logic is:

| Case | Grouping key used | Road label generated |
|------|-------------------|----------------------|
| Road has `FULL_STREET_NAME` | `named::{FULL_STREET_NAME}` | `{FULL_STREET_NAME}` |
| Road has no official street name but has `ROUTE_NUMBER` | `unnamed_highway_route::{ROUTE_NUMBER}` | `Unnamed {ROAD_CLASS} Route {ROUTE_NUMBER}` |
| Road has no official street name but has `ROUTE_NAME_ENGLISH` | `unnamed_highway_route_name::{ROUTE_NAME_ENGLISH}` | `Unnamed {ROAD_CLASS} Route {ROUTE_NAME_ENGLISH}` |
| Road has no official street name but has a highway-like road class | `unnamed_highway_element::{ROAD_CLASS}_{OGF_ID}` | `Unnamed {ROAD_CLASS} Road Element {OGF_ID}` |
| Road has no official street name and is not highway-like | Not mapped as a Road | Excluded from the Road grouping |

A road element is considered highway-like if its road class is `Freeway`, `Expressway / Highway`, or `Ramp`, or if it has a route name or route number. A unique URI is generated for each Road entity, and all corresponding RoadLinks are created and linked to that Road using the partwhole:hasProperPart property.

**Why this was changed:** Previously, grouping only by `FULL_STREET_NAME` meant that unnamed roads were skipped before Road and RoadLink entities could be created. The updated strategy preserves unnamed highway-like infrastructure while still avoiding low-value unnamed local elements.
...
Each RoadLink is assigned a readable display name. If the road element has an official street name, the display name uses that value. If the road element is unnamed but highway-like, the display name is generated from the route number, route name, or road class. The mapping also asserts `cdt:isUnnamedRoadElement` as a boolean flag so that generated labels are not confused with official street names.
...
<tr class="even">
<td rowspan="4">OGF_ID</td>
<td>orn:roadLink_{OGF_ID}</td>
<td>genprop:hasIdentifier</td>
<td>{OGF_ID}</td>
<td rowspan="4">A unique numeric provincial identifier assigned to each object. The display name is generated from the official street name when available, otherwise from route information or road class for unnamed highway-like elements.</td>
</tr>
<tr class="odd">
<td>orn:roadLink_{OGF_ID}</td>
<td>rdf:type</td>
...
<td>orn:roadLink_{OGF_ID}</td>
<td>genprop:hasName</td>
<td>{ROAD_LINK_DISPLAY_NAME} (Road Element {OGF_ID})</td>
</tr>
<tr class="odd">
<td>orn:roadLink_{OGF_ID}</td>
<td>cdt:isUnnamedRoadElement</td>
<td>xsd:boolean</td>
</tr>
<tr class="odd">
<td>TO_JCT</td>
...
|------------------------------------------------|------------------------|-----------------|---------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Field Name**                                 | **Subject**            | **Property**    | **Object**          | **Notes**                                                                                                                                                                                                                        |
| FULL_STREET_NAME                               | orn:roadLink\_{OGF_ID} | genProp:hasName | {ROAD_LINK_DISPLAY_NAME} (Road Element {OGF_ID}) | If `FULL_STREET_NAME` is present, it is used as the display name. If it is missing and the road is highway-like, a generated display name is used from route number, route name, or road class. |

*<u>**Table 7**: Mapping ORN_OFFICIAL_STREET_NAME.csv Road Link data to City Digital Twin</u>*


**Updated unnamed road and highway handling:** Some ORN road net elements have missing or non-informative `FULL_STREET_NAME` values. In the updated mapping, those elements are no longer automatically dropped. Instead, unnamed road elements are mapped if they are highway-like.

| Mapping rule | Subject | Property / output | Notes |
|--------------|---------|-------------------|-------|
| Missing official street name | orn:roadLink\_{OGF_ID} | cdt:isUnnamedRoadElement true | Indicates that the road link label was generated and was not taken from `FULL_STREET_NAME`. |
| Present official street name | orn:roadLink\_{OGF_ID} | cdt:isUnnamedRoadElement false | Indicates that the road link has an official street name. |
| Highway-like unnamed element with route number | orn:roadLink\_{OGF_ID} | genprop:hasName `Unnamed {ROAD_CLASS} Route {ROUTE_NUMBER} (Road Element {OGF_ID})` | Keeps named access to highways even when `FULL_STREET_NAME` is absent. |
| Highway-like unnamed element with route name | orn:roadLink\_{OGF_ID} | genprop:hasName `Unnamed {ROAD_CLASS} Route {ROUTE_NAME_ENGLISH} (Road Element {OGF_ID})` | Used when route number is unavailable but route name is available. |
| Highway-like unnamed element with no route name or route number | orn:roadLink\_{OGF_ID} | genprop:hasName `Unnamed {ROAD_CLASS} Road Element {OGF_ID}` | Used as a fallback for road classes such as Freeway, Expressway / Highway, or Ramp. |

*<u>**Table 7a**: Additional mapping rules for unnamed highway-like ORN road elements</u>*
...
|-------------------------------------|------------------------|---------------|----------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Field Name**                      | **Subject**            | **Property**  | **Object**           | **Notes**                                                                                                                                                                                 |
| ROUTE_NAME_ENGLISH                  | orn:roadLink\_{OGF_ID} | cdt:routeName | {ROUTE_NAME_ENGLISH} | The English name that is attached to a road net element as defined by a Municipality, Provincial Ministry, or Federal Agency and is associated to an established and/or maintained route. This value is also used to retain and label unnamed highway-like road elements when `FULL_STREET_NAME` is missing. |
...
The merged road network data is grouped by a generated `road_group_key`, not only by `FULL_STREET_NAME_road_names`. Named roads use `FULL_STREET_NAME`, while unnamed highway-like elements use route number, route name, or road class plus OGF_ID. Each group becomes a Road instance, and the associated RoadLink instances are later attached to it using partwhole:hasProperPart. This models a road as a collection of road links sharing the same official name, route identifier, or generated unnamed-highway grouping key.

The updated logic prevents unnamed highways, expressways, and ramps from being lost at the grouping stage.
...
- unnamed-road boolean flag (`cdt:isUnnamedRoadElement`)
...
- Roads are grouped by a generated road-group key. Named roads use `FULL_STREET_NAME`; unnamed highway-like elements are retained using route number, route name, or road class plus OGF_ID. Unnamed non-highway-like elements are still excluded from Road grouping.
...
## ORN mapping updates added for unnamed roads and highways

The following updates were made to reflect the revised ORN mapping script:

1. **Unnamed highway-like roads are retained.** Road elements with missing official street names are no longer automatically removed. They are retained if they are highway-like, meaning their `ROAD_CLASS` is `Freeway`, `Expressway / Highway`, or `Ramp`, or they contain a `ROUTE_NAME_ENGLISH` or `ROUTE_NUMBER` value.

2. **Road grouping now uses `road_group_key`.** The grouping key uses `FULL_STREET_NAME` for named roads, `ROUTE_NUMBER` for unnamed route-numbered roads, `ROUTE_NAME_ENGLISH` for unnamed route-named roads, and `ROAD_CLASS + OGF_ID` as a fallback for unnamed highway-like elements.

3. **Generated display labels are documented.** Unnamed elements are labelled with generated names such as `Unnamed Expressway / Highway Route 401 (Road Element 12345)`. This improves readability while preserving the fact that the official street name was missing.

4. **`cdt:isUnnamedRoadElement` is documented.** Each RoadLink now receives a boolean flag indicating whether the name was generated because the official street name was missing.

5. **The old assumption that all unnamed roads are skipped was removed.** The documentation now states that only unnamed non-highway-like road elements are excluded from Road grouping.
